...
async function fetchAndDisplayEvents() {
    try {
        const events = await fetchData("data/member.json");
        if (events) {
...
async function fetchBusinessHighlights() {
    try {
        highlights = await fetchData('data/member.json');
...
function DisplayResults({ main,weather }) {
    ...
    temperature.innerHTML = `Temperature: ${main.temp}&deg;F`;
...
function Forecast({ list }) {
    ...
    list.filter((_, index) => index % 8 === 0).forEach(dayForecast => {
        ...
        forecastCard.innerHTML = `
            ...
            <p style="color: blue;">Temperature: ${Math.round(temp)} °C</p>
        `;
        forecastContainer.appendChild(forecastCard);
    });
}

// Botón para ver más destacados
